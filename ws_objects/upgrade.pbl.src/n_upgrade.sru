$PBExportHeader$n_upgrade.sru
$PBExportComments$File Upgrade
forward
global type n_upgrade from datastore
end type
type filetime from structure within n_upgrade
end type
type systemtime from structure within n_upgrade
end type
type win32_find_data from structure within n_upgrade
end type
end forward
...
//	If ls_filename = "." Or ls_filename = ".." Or Then
	If Right(ls_filename, 3) = "exe" Or Right(ls_filename, 3) = "pbd" Or Right(ls_filename, 3) = "dll" Or Right(ls_filename, 3) = "DLL" Or ls_filename = "Favorite.ico" Or Right(ls_filename, 3) = "pbx" Or Right(ls_filename, 3) = "gif" Or Right(ls_filename, 3) = "jpg"Then
		// check for hidden attrib
		lb_hidden = of_checkbit(lstr_fd.dwFileAttributes, 2)
		lb_system = of_checkbit(lstr_fd.dwFileAttributes, 3)
		lb_subdir = of_checkbit(lstr_fd.dwFileAttributes, 5)
